...
void ESP32_FTPClient::WriteClientBuffered(WiFiClient* cli,  char * data, int dataLength) {
  if(!isConnected()) return;
...
void ESP32_FTPClient::WriteData ( char* data, int dataLength) {
  FTPdbgn(F("Writing"));
  if(!isConnected()) return;
...
void ESP32_FTPClient::Write(String str) {
...
void ESP32_FTPClient::AppendFile (const char* fileName) {
  FTPdbgn("Send APPE");
  if(!isConnected()) return;
  client.print(F("APPE "));
  client.println(F(fileName));
